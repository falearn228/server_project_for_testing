...
1. [**Общие понятия QoS:**](#1-общие-понятия-qos)
    ...
    - [Что такое QoS и зачем он нужен?](#что-такое-qos-и-зачем-он-нужен)
    - [Какие параметры влияют на качество обслуживания в сетях](#какие-параметры-влияют-на-качество-обслуживания-в-сетях)
2. [**Модели обеспечения QoS:**](#2-модели-обеспечения-qos)
    
    - [Best Effort (лучшая попытка)](#best-effort-лучшая-попытка)
    - [Integrated Services (IntServ)](#integrated-services-intserv)
    - [Differentiated Services (DiffServ)](#differentiated-services-diffserv)
3. [**Механизмы DiffServ:**](#3-механизмы-diffserv)
    
    - [Маркировка и классификация трафика](#маркировка-и-классификация-трафика)
    - [Как работают механизмы классификации, например, ACL (Access Control List)](#как-работают-механизмы-классификации-например-acl-access-control-list)
    - [PHB (Per-Hop Behavior) и типы обслуживания (EF, AF, BE)](#phb-per-hop-behavior-и-типы-обслуживания-ef-af-be)
4. [**Очереди и управление трафиком:**](#4-очереди-и-управление-трафиком)
    
    - [Очереди FIFO, PQ, FQ, RR](#очереди-fifo-pq-fq-rr)
    - [Различия между алгоритмами очередей и их применение на практике](#различия-между-алгоритмами-очередей-и-их-применение-на-практике)
    - [Сравнение простоты реализации и эффективности различных алгоритмов](#сравнение-простоты-реализации-и-эффективности-различных-алгоритмов)
    - [Итог по механизмам диспетчеризации: где и как используются](#итог-по-механизмам-диспетчеризации-где-и-как-используются)
5. [**Предотвращение и управление перегрузками:**](#5-предотвращение-и-управление-перегрузками)
    
    - [RED (Random Early Detection) и WRED (Weighted RED)](#red-random-early-detection-и-wred-weighted-red)
    - [Tail Drop](#tail-drop)
    - [Политики управления перегрузками: что лучше для разных сценариев](#политики-управления-перегрузками-что-лучше-для-разных-сценариев)
6. [**Ограничение скорости и шейпинг:**](#6-ограничение-скорости-и-шейпинг)
    
    - [Алгоритмы Traffic Policing и Traffic Shaping](#алгоритмы-traffic-policing-и-traffic-shaping)
    - [Токен-бакет (Token Bucket) и Личный-бакет (Leaky Bucket): различия, примеры применения](#токен-бакет-token-bucket-и-личный-бакет-leaky-bucket-различия-примеры-применения)
    - [Настройка шейпинга в сетевых устройствах](#настройка-шейпинга-в-сетевых-устройствах)
7. [**Аппаратная и программная реализация QoS:**](#7-аппаратная-и-программная-реализация-qos)
    
    - [Как современные коммутаторы и маршрутизаторы реализуют QoS на аппаратном уровне](#как-современные-коммутаторы-и-маршрутизаторы-реализуют-qos-на-аппаратном-уровне)
    - [Примеры команд настройки QoS для оборудования Cisco, Huawei, MikroTik](#примеры-команд-настройки-qos-для-оборудования-cisco-huawei-mikrotik)
	* [Ограничения QoS в виртуализированных средах](#ограничения-qos-в-виртуализированных-средах-например-sd-wan)
8. [**Ссылки на литературу](#8-ссылки-на-литературу)
